...
// 1 -Function qui permet de récupérer les messages et de les afficher correctement
	function getMessages(){
		...
		requeteAjax.onload = function() { //Quand la réponse du server est chargée
			...
			const html = results.map(function(message){ // Un objet Map permet de retrouver ses éléments dans leur ordre d'insertion
			const $time = message.created_at.slice(11, message.created_at.length);//  extrait une section d'une chaine de caractères
				...
				`
			}).join(''); //  les éléments seront accolés les uns aux autres sans espace entre. 
